...
그런데 매일 자정 캠페인 데이터를 리셋하는 기능이 참조하는 원본 파일 `db.json`이 `server/` 안이 아니라 저장소 맨 위에 있습니다. Root Directory를 `server/`로 잡으면 Render는 그 폴더만 가져오니, 바깥에 있는 `db.json`은 서버 입장에서 없는 파일이 됩니다. 배포는 멀쩡히 성공해서 겉으로는 문제가 없어 보이지만, 리셋 기능을 실제로 실행하는 순간 파일을 못 찾아 조용히 죽었을 상황이었습니다.
...
### 4. 슬립 방지용 핑이 DB를 계속 깨워놔서 무료 compute 한도를 태웠습니다

위 3번으로 Render 슬립은 잡았는데, 한 달쯤 뒤에 사이트가 통째로 죽었습니다. 증상이 묘했습니다 — 프론트(Vercel)는 멀쩡히 뜨고, API도 살아서 응답은 하는데, DB를 건드리는 라우트만 전부 500이었습니다.

```
GET  /            → 401  (JWT 가드 동작, DB 미접근)   ← 정상
GET  /health      → 500  (SELECT 1)                  ← DB 접근
POST /auth/login  → 500  (user 조회)                 ← DB 접근
```

서버가 죽은 게 아니라 DB 연결만 끊긴 상태였고, Neon 콘솔이 이유를 알려줬습니다. **이번 달 compute 한도 소진.**

범인은 3번에서 붙인 헬스체크 핑이었습니다. UptimeRobot이 5분마다 때리던 `/health`가 `SELECT 1`을 날리고 있었습니다. Neon 무료 티어는 일정 시간 쿼리가 없으면 compute를 자동으로 0으로 내리는데(scale to zero), 5분 간격 핑이 그걸 매번 리셋시키니 **DB가 24시간 내내 깨어 있었습니다.** Render 무료 한도(750시간)만 계산하고 Neon의 compute 시간 한도는 아예 검토 대상에 없었던 게 구멍이었습니다.

정작 Render 슬립을 막는 데 DB 쿼리는 전혀 필요가 없었습니다. 프로세스가 요청을 받기만 하면 됩니다. 그래서 헬스체크를 두 개로 쪼갰습니다.

| 엔드포인트 | 용도 | DB 접근 |
|---|---|---|
| `GET /health/live` | liveness — 외부 모니터가 5분마다 핑 | ❌ |
| `GET /health` | readiness — DB까지 실제로 도는지 수동 확인 | ✅ |

UptimeRobot 모니터 URL을 `/health/live`로 바꾸면 Render는 계속 깨어 있고, Neon compute는 실제 방문자가 있을 때만 올라옵니다. 나중에 누가 liveness에 "이왕이면 DB도 확인하자"고 `SELECT 1`을 다시 넣으면 같은 사고가 재발하므로, 그러지 못하도록 컨트롤러 주석과 테스트(`health.controller.spec.ts` — DB가 죽은 상태를 mock으로 만들어 liveness가 여전히 200인지 검증)로 못을 박아뒀습니다.
