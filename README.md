...
요구사항에는 없었으나, 실제 사용자의 편의성과 기술적 완성도를 높이기 위해 아래 기능을 추가로 구현했습니다.

- **다크 모드 / 라이트 모드 지원**: Tailwind CSS 4의 테마 시스템을 활용하여 가독성 높은 디자인을 제공하며, 차트 라이브러리(Recharts)와 연동하여 테마별 최적화된 시각화를 제공합니다.
- **캠페인 삭제 및 일괄 관리**: 요구사항에 명시되지 않았으나, 실제 운영 환경에 필수적인 **삭제(Delete)** 기능을 추가하고 이를 확장하여 **전체 선택/일괄 삭제/일괄 상태 변경**까지 가능하도록 구현했습니다.
- **검색 결과 실시간 카운팅**: 전체 데이터 대비 검색된 데이터의 개수를 실시간으로 시각화하여 데이터 필터링 상태를 직관적으로 파악할 수 있도록 배려했습니다.
...
- **전역 상수 관리 (Magic Number Refactoring)**: 페이지네이션 설정, 차트 디자인 수치(마진, 반지름), UI 치수 등을 `shared/constants/` 하위의 전문 상수 파일로 분리하여 한 곳에서 관리합니다. 이를 통해 디자인 시스템 변경에 유연하게 대응하고 코드의 가독성을 극대화했습니다.
