...
### 사례 5: 유지보수성을 고려한 매직 넘버(Magic Number) 전역 리팩토링

[문제 상황]
UI 치수(마진, 너비, 높이), 페이지네이션 제한 수치, 차트의 세부 스타일(반지름, 폰트 크기) 등이 컴포넌트 내부에 하드코딩되어 있었습니다. 이는 디자인 시스템 변경이나 비즈니스 정책 변경 시 모든 파일을 찾아 수정해야 하는 잠재적 위험 요인이었습니다.

[수정 및 판단]

- **전역 상수화**: `shared/constants/` 디렉토리에 `pagination.ts`, `chart.ts`, `ui.ts`, `campaign.ts` 등으로 나누어 모든 매직 넘버를 추출했습니다.
- **가독성 향상**: 단순히 `10`, `9`, `24`와 같은 숫자 대신 `PAGINATION.ITEMS_PER_PAGE`, `CHART_CONFIG.TOP_RANKING.BAR_SIZE`와 같은 명칭을 사용하여 코드의 의도를 명확히 했습니다.
- **중앙 집중식 관리**: 이제 차트의 마진이나 테이블의 컬럼 개수를 변경하고 싶을 때 단 한 곳의 상수 파일만 수정하면 전체 코드가 동기화되는 견고한 구조를 갖추게 되었습니다.
...
### 4-2. 실시간 데이터 정확성 확보 — `no-store` 전략

초기 설계에서는 Next.js 16의 `force-cache`와 `updateTag`를 활용한 성능 최적화를 고려했으나, 로컬 개발 환경(`json-server`) 및 과제 검토 환경의 특수성을 고려하여 **항상 최신 데이터를 보장하는 `no-store` 방식**으로 최종 결정했습니다.

- **이유**: `json-server`를 사용하는 환경에서는 외부에서 `db.json`을 수동으로 수정하는 상황이 빈번합니다. `force-cache`를 사용할 경우 서버 액션을 통하지 않은 수동 수정 사항이 UI에 즉시 반영되지 않아 혼란을 줄 수 있습니다.
- **구현**: 모든 fetch 요청에서 불필요한 캐시 옵션을 제거했습니다. Next.js 15+ 환경에서는 특별한 설정이 없는 fetch는 기본적으로 `no-store`(Dynamic)로 동작하므로, 명시적인 `{ cache: 'no-store' }` 조차 삭제하여 코드를 더욱 간결하게 유지했습니다.
- **결과**: 불필요한 설정값과 복잡한 캐시 무효화(`revalidateTag`, `updateTag`) 로직을 모두 걷어내어, 가장 단순하면서도 로컬 데이터와 100% 동기화되는 구조를 완성했습니다.
