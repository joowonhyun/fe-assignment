...
### 사례 4: 실무 환경을 고려한 기능 확장 (다크모드 & 삭제 기능 추가)

[문제 상황]
기본 요구사항은 단순 조회 및 등록, 필터링 작업이었으나, 실제 마케터가 상시 사용하는 대시보드라면 장시간 사용 시 눈의 피로도를 줄여주는 **다크 모드**와 잘못 등록된 데이터를 정리하기 위한 **삭제(Delete)** 기능이 필수적이라고 판단했습니다.

[수정 및 판단]
- **Dark Mode**: 단순히 배경색만 바꾸는 것이 아니라, Recharts와 같은 시각화 라이브러리 내부의 텍스트와 그리드 색상까지 다크 모드에 최적화되도록 전역 테마 상태에 따라 차트 옵션을 동적으로 매핑했습니다.
- **삭제 및 일괄 관리**: 요구사항에 없는 '삭제' 기능을 주도적으로 추가했으며, 나아가 `Set` 자료구조를 활용한 **전체 선택/일괄 삭제** 로직을 직접 설계하여 운영 효율성까지 고려한 완성도 높은 UI/UX를 구현했습니다.
